...
NUM_EPOCHS = 4
NUM_MINIBATCHES = 8
TOTAL_UPDATES = 5000    # training updates
LR = 3e-4
GAMMA = 0.995
GAE_LAMBDA = 0.95
CLIP_EPS = 0.2
...
def train():
    ...
    def collect_and_update(states, pp, sp, cp, po, so, co, key):
        ...
        def env_step(carry, _):
            ...
            def sample_ped(logits, key):
                ...
                return action, lp
            ped_actions, ped_lp = jax.vmap(jax.vmap(sample_ped))(ped_logits, ped_keys)  # (E,P), (E,P)
    ...
    for i in range(1, TOTAL_UPDATES + 1):
        ...
        if i % 500 == 0 or i == TOTAL_UPDATES:
            ...
            path = os.path.join(CHECKPOINT_DIR, f"decisions_{i}.pkl")
    ...
    final_path = os.path.join(CHECKPOINT_DIR, "decisions_final.pkl")
